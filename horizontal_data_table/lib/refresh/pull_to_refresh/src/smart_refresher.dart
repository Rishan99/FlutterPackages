...
class SmartRefresherState extends State<SmartRefresher> {
  ...
  ScrollPhysics _getScrollPhysics(RefreshConfiguration? conf, ScrollPhysics physics) {
    final bool isBouncingPhysics =
        physics is BouncingScrollPhysics || (physics is AlwaysScrollableScrollPhysics && ScrollConfiguration.of(context).getScrollPhysics(context).runtimeType == BouncingScrollPhysics);
  ...
  Widget? _buildBodyBySlivers(Widget? childView, List<Widget>? slivers, RefreshConfiguration? conf) {
    ...
    if (childView is! Scrollable) {
      ...
      if (childView is ScrollView) {
        ...
        widget.controller.scrollController = scrollController ?? childView.controller ?? (childView.primary == true ? PrimaryScrollController.of(context) : null);
      } else {
        // ignore: DEPRECATED_MEMBER_USE_FROM_SAME_PACKAGE
...
class RefreshController {
  ...
  Future<void>? requestRefresh({bool needMove = true, Duration duration = const Duration(milliseconds: 500), Curve curve = Curves.linear}) {
  ...
  Future<void>? requestLoading({bool needMove = true, Duration duration = const Duration(milliseconds: 300), Curve curve = Curves.linear}) {
