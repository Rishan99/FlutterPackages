...
class ConcreteSeriesLegend<D> extends SeriesLegend<D> {
  ConcreteSeriesLegend({SelectionModelType selectionModelType, LegendEntryGenerator<D> legendEntryGenerator})
    : super(selectionModelType: selectionModelType, legendEntryGenerator: legendEntryGenerator);
...
void main() {
  ...
  setUp(() {
    series1 = MutableSeries(Series<MyRow, String>(id: 's1', data: [s1D1, s1D2, s1D3], domainFn: (MyRow row, _) => row.campaign, measureFn: (MyRow row, _) => row.count, colorFn: (_, __) => blue));

    series2 = MutableSeries(Series<MyRow, String>(id: 's2', data: [s2D1, s2D2, s2D3], domainFn: (MyRow row, _) => row.campaign, measureFn: (MyRow row, _) => row.count, colorFn: (_, __) => red));
  });
  ...
  test('default hidden series are removed from list during pre process', () {
    final seriesList = [series1, series2];
    final selectionType = SelectionModelType.info;
    final legend = ConcreteSeriesLegend<String>(selectionModelType: selectionType);
  ...
  test('generated legend entries use provided formatters', () {
    final seriesList = [series1, series2];
    final selectionType = SelectionModelType.info;
    final measureFormatter = (num value) => 'measure ${value.toStringAsFixed(0)}';
    final secondaryMeasureFormatter = (num value) => 'secondary ${value.toStringAsFixed(0)}';
    final legend = SeriesLegend<String>(selectionModelType: selectionType, measureFormatter: measureFormatter, secondaryMeasureFormatter: secondaryMeasureFormatter);
    ...
    chart.callOnPreProcess();
    chart.callOnPostProcess();
    chart.getSelectionModel(selectionType).updateSelection([SeriesDatum(series1, s1D1), SeriesDatum(series2, s2D1)], [series1, series2]);
  ...
  test('series legend - show measure sum when there is no selection', () {
    ...
    final legend = SeriesLegend<String>(selectionModelType: selectionType, legendDefaultMeasure: LegendDefaultMeasure.sum, measureFormatter: measureFormatter);
  ...
  test('series legend - show measure average when there is no selection', () {
    ...
    final selectionType = SelectionModelType.info;
    final measureFormatter = (num value) => '${value.toStringAsFixed(0)}';
    final legend = SeriesLegend<String>(selectionModelType: selectionType, legendDefaultMeasure: LegendDefaultMeasure.average, measureFormatter: measureFormatter);
  ...
  test('series legend - show first measure when there is no selection', () {
    ...
    final selectionType = SelectionModelType.info;
    final measureFormatter = (num value) => '${value.toStringAsFixed(0)}';
    final legend = SeriesLegend<String>(selectionModelType: selectionType, legendDefaultMeasure: LegendDefaultMeasure.firstValue, measureFormatter: measureFormatter);
  ...
  test('series legend - show last measure when there is no selection', () {
    ...
    final selectionType = SelectionModelType.info;
    final measureFormatter = (num value) => '${value.toStringAsFixed(0)}';
    final legend = SeriesLegend<String>(selectionModelType: selectionType, legendDefaultMeasure: LegendDefaultMeasure.lastValue, measureFormatter: measureFormatter);
