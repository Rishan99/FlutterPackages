...
void main() {
  ...
  setUp(() {
    ...
    domainAxis = MockAxis();
    _series1 = MutableSeries(Series<MyRow, String>(id: 's1', data: [_s1D1, _s1D2, _s1D3], domainFn: (MyRow row, _) => row.campaign, measureFn: (MyRow row, _) => row.count))
  ...
  test('nodes ordered correctly with a series missing a domain', () {
    ...
    final seriesWithMissingDomain = MutableSeries(Series<MyRow, String>(id: 'm1', data: [_s1D1, _s1D3], domainFn: (MyRow row, _) => row.campaign, measureFn: (MyRow row, _) => row.count))
      ..setAttr(domainAxisKey, domainAxis);
