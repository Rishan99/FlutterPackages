...
class FakeRenderer<D> extends BaseSeriesRenderer<D> {
  ...
  @override
  List<DatumDetails<D>> getNearestDatumDetailPerSeries(Point<double> chartPoint, bool byDomain, Rectangle<int> boundsOverride, {selectOverlappingPoints = false, selectExactEventLocation = false}) =>
      null;
...
void main() {
  ...
  InitialSelection _makeBehavior(SelectionModelType selectionModelType, {List<String> selectedSeries, List<SeriesDatumConfig> selectedData}) {
    InitialSelection behavior = InitialSelection(selectionModelType: selectionModelType, selectedSeriesConfig: selectedSeries, selectedDataConfig: selectedData);
  ...
  setUp(() {
    ...
    _series1 = MutableSeries(Series(id: 'mySeries1', data: ['A', 'B', 'C', 'D'], domainFn: (dynamic datum, __) => datum, measureFn: (_, __) => null));

    _series2 = MutableSeries(Series(id: 'mySeries2', data: ['W', 'X', 'Y', 'Z'], domainFn: (dynamic datum, __) => datum, measureFn: (_, __) => null));

    _series3 = MutableSeries(Series(id: 'mySeries3', data: ['W', 'X', 'Y', 'Z'], domainFn: (dynamic datum, __) => datum, measureFn: (_, __) => null));

    _series4 = MutableSeries(Series(id: 'mySeries4', data: ['W', 'X', 'Y', 'Z'], domainFn: (dynamic datum, __) => datum, measureFn: (_, __) => null));
  });

  test('selects initial datum', () {
    _makeBehavior(infoSelectionType, selectedData: [SeriesDatumConfig('mySeries1', 'C')]);
  ...
  test('selects multiple initial data', () {
    _makeBehavior(infoSelectionType, selectedData: [SeriesDatumConfig('mySeries1', 'C'), SeriesDatumConfig('mySeries1', 'D')]);
  ...
  test('selects multiple series', () {
    _makeBehavior(infoSelectionType, selectedSeries: ['mySeries2', 'mySeries4']);
  ...
  test('selects series and datum', () {
    _makeBehavior(infoSelectionType, selectedData: [SeriesDatumConfig('mySeries1', 'C')], selectedSeries: ['mySeries4']);
  ...
  test('selection model is reset when a new series is drawn', () {
    ...
    _chart.draw([
      Series(id: 'mySeries2', data: ['W', 'X', 'Y', 'Z'], domainFn: (dynamic datum, __) => datum, measureFn: (_, __) => null),
    ]);
