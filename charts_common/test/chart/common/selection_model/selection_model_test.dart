...
void main() {
  ...
  setUp(() {
    ...
    _closestSeries = MutableSeries<String>(Series<MyDatum, String>(id: 'closest', data: [_closestDatumClosestSeries, _otherDatumClosestSeries], domainFn: (d, _) => d.id, measureFn: (_, __) => 0));
    _closestDatumClosestSeriesPair = SeriesDatum<String>(_closestSeries, _closestDatumClosestSeries);
    _otherDatumClosestSeriesPair = SeriesDatum<String>(_closestSeries, _otherDatumClosestSeries);

    _closestDatumOtherSeries = MyDatum('cDoS');
    _otherDatumOtherSeries = MyDatum('oDoS');
    _otherSeries = MutableSeries<String>(Series<MyDatum, String>(id: 'other', data: [_closestDatumOtherSeries, _otherDatumOtherSeries], domainFn: (d, _) => d.id, measureFn: (_, __) => 0));
    _closestDatumOtherSeriesPair = SeriesDatum<String>(_otherSeries, _closestDatumOtherSeries);
    _otherDatumOtherSeriesPair = SeriesDatum<String>(_otherSeries, _otherDatumOtherSeries);
  });
  ...
  group('SelectionModel persists values', () {
    ...
    test('all datum are selected but only the first Series is', () {
      ...
      _selectionModel.updateSelection([SeriesDatum(_closestSeries, _closestDatumClosestSeries), SeriesDatum(_otherSeries, _closestDatumOtherSeries)], [_closestSeries]);

      expect(_selectionModel.hasDatumSelection, isTrue);
      expect(_selectionModel.selectedDatum, hasLength(2));
      expect(_selectionModel.selectedDatum, contains(_closestDatumClosestSeriesPair));
      expect(_selectionModel.selectedDatum, contains(_closestDatumOtherSeriesPair));
      expect(_selectionModel.selectedDatum.contains(_otherDatumClosestSeriesPair), isFalse);
      expect(_selectionModel.selectedDatum.contains(_otherDatumOtherSeriesPair), isFalse);
    ...
    test('selection can change', () {
      ...
      _selectionModel.updateSelection([SeriesDatum(_otherSeries, _otherDatumOtherSeries)], [_otherSeries]);
  ...
  group('SelectionModel changed listeners', () {
    test('listener triggered for change', () {
      ...
      // Listen
      _selectionModel.addSelectionChangedListener((SelectionModel<String> model) {
        triggeredModel = model;
      });

      // Set the selection to closest datum.
      _selectionModel.updateSelection([SeriesDatum(_closestSeries, _closestDatumClosestSeries)], [_closestSeries]);
  ...
  group('SelectionModel updated listeners', () {
    test('listener triggered for change', () {
      SelectionModel<String> triggeredModel;
      // Listen
      _selectionModel.addSelectionUpdatedListener((SelectionModel<String> model) {
        triggeredModel = model;
      });
  ...
  group('SelectionModel locked listeners', () {
    test('listener triggered when model is locked', () {
      SelectionModel<String> triggeredModel;
      // Listen
      _selectionModel.addSelectionLockChangedListener((SelectionModel<String> model) {
        triggeredModel = model;
      });
