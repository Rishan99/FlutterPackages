...
import 'package:charts_common/src/chart/pie/arc_label_decorator.dart' show ArcLabelPosition;
import 'package:charts_common/src/chart/sunburst/sunburst_arc_label_decorator.dart' show SunburstArcLabelDecorator;
import 'package:charts_common/src/chart/pie/arc_renderer_element.dart' show ArcRendererElementList;
import 'package:charts_common/src/chart/sunburst/sunburst_arc_renderer.dart' show SunburstArcRendererElement;
...
void main() {
  ...
  group('sunburst chart', () {
    test('Paint labels with default settings', () {
      ...
      decorator.decorate([arcElements], canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0);
      ...
      expect(captured[2], equals(100 - decorator.insideLabelStyleSpec.fontSize ~/ 2));
      // For arc 'B'.
      expect(captured[3].maxWidth, equals(20));
      expect(captured[3].textDirection, equals(TextDirection.rtl));
      expect(captured[4], equals(60 - decorator.leaderLineStyleSpec.length - decorator.labelPadding * 3));
      expect(captured[5], equals(100 - decorator.outsideLabelStyleSpec.fontSize ~/ 2));
      ...
      expect(captured[8], equals(100 - decorator.insideLabelStyleSpec.fontSize ~/ 2));
    });
    ...
    test('setting outerRingArcLabelPosition inside paints inside the arc', () {
      ...
      final decorator = SunburstArcLabelDecorator(outerRingArcLabelPosition: ArcLabelPosition.inside, insideLabelStyleSpec: TextStyleSpec(fontSize: 10));
    ...
    test('LabelPosition.outside paints outside the arc for the outer most '
      ...
      expect(captured[1], equals(140 + decorator.leaderLineStyleSpec.length + decorator.labelPadding * 3));
      expect(captured[2], equals(100 - decorator.outsideLabelStyleSpec.fontSize ~/ 2));
    });
    ...
    test('Inside and outside label styles are applied', () {
      ...
      expect(captured[4], equals(50 - decorator.leaderLineStyleSpec.length - decorator.labelPadding * 3));
      expect(captured[5], equals(100 - decorator.outsideLabelStyleSpec.fontSize ~/ 2));
    });
  });
  ...
  group('Null and empty label scenarios', () {
    test('Skip label if label accessor does not exist', () {
      ...
      SunburstArcLabelDecorator().decorate([arcElements], canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0);
