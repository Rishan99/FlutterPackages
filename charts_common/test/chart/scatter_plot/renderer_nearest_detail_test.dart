...
void main() {
  ...
  MutableSeries<num> _makeSeries({String id, String seriesCategory}) {
    ...
    final series = MutableSeries(
      Series<MyRow, num>(id: id, data: data, radiusPxFn: (row, _) => row.radius, domainFn: (row, _) => row.campaign, measureFn: (row, _) => row.clickCount, seriesCategory: seriesCategory),
    );
  ...
  group('getNearestDatumDetailPerSeries', () {
    test('with both selectOverlappingPoints and selectOverlappingPoints set to '
      ...
      final seriesList = <MutableSeries<num>>[
        _makeSeries(id: 'foo')..data.addAll(<MyRow>[MyRow('point1', 20, 30, 6, 0, ''), MyRow('point2', 15, 20, 3, 0, ''), MyRow('point3', 30, 40, 4, 0, '')]),
      ];
    ...
    test('with both selectOverlappingPoints and selectOverlappingPoints set to '
      ...
      final renderer = PointRenderer(config: PointRendererConfig())..layout(layout, layout);
      final seriesList = <MutableSeries<num>>[
        _makeSeries(id: 'foo')..data.addAll(<MyRow>[MyRow('point1', 15, 30, 15, 0, ''), MyRow('point2', 10, 20, 5, 0, ''), MyRow('point3', 30, 40, 4, 0, '')]),
      ];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
    ...
    test('with both selectOverlappingPoints and selectOverlappingPoints set to '
      ...
      final renderer = PointRenderer(config: PointRendererConfig())..layout(layout, layout);
      final seriesList = <MutableSeries<num>>[
        _makeSeries(id: 'foo')..data.addAll(<MyRow>[MyRow('point1', 15, 30, 2, 0, ''), MyRow('point2', 10, 20, 3, 0, ''), MyRow('point3', 30, 40, 4, 0, '')]),
      ];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
    ...
    test('with both selectOverlappingPoints == true and '
    ...
    test('with both selectOverlappingPoints == false and '
