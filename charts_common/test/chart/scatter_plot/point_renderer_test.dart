...
void main() {
    ...
    numericSeriesList = [
      MutableSeries<int>(
        Series<MyRow, int>(
          ...
          ..setAttribute(boundsLineRadiusPxFnKey, (int index) => myFakeDesktopData[index].boundsRadius),
      ),
    ];
  });
  ...
  group('preprocess', () {
    ...
    test('with numeric data and missing radiusPxFn', () {
      renderer = PointRenderer<int>(config: PointRendererConfig(radiusPx: 2.0, boundsLineRadiusPx: 1.5));
    ...
    test('with custom symbol renderer ID in data', () {
      ...
      numericSeriesList[0].setAttr(pointSymbolRendererFnKey, (int index) => numericSeriesList[0].data[index].shape as String);
