...
void main() {
  ...
  group('Find closest step size from stepper', () {
    test('from exactly matching step size', () {
      final stepper = AutoAdjustingDateTimeTickProvider.createHourTickProvider(dateTimeFactory);
    ...
    test('choose smallest increment if step is smaller than smallest increment', () {
      final stepper = AutoAdjustingDateTimeTickProvider.createHourTickProvider(dateTimeFactory);
      final oneHourMs = (Duration(hours: 1)).inMilliseconds;
      final closestStepSize = stepper.getClosestStepSize((Duration(minutes: 56)).inMilliseconds);
    ...
    test('choose largest increment if step is larger than largest increment', () {
      ...
      final closestStepSize = stepper.getClosestStepSize((Duration(hours: 25)).inMilliseconds);
    ...
    test('choose closest increment if exact not found', () {
      ...
      final closestStepSize = stepper.getClosestStepSize((Duration(hours: 3, minutes: 28)).inMilliseconds);
