...
import 'package:charts_common/src/chart/bar/bar_label_decorator.dart' show BarLabelDecorator, BarLabelAnchor, BarLabelPlacement, BarLabelPosition, BarLabelVerticalPosition;
...
class FakeTextElement implements TextElement {
  ...
  @override
  TextMeasurement get measurement => TextMeasurement(horizontalSliceWidth: text.length.toDouble(), verticalSliceWidth: textStyle.fontSize.toDouble(), baseline: textStyle.fontSize.toDouble());
}
...
void main() {
  ...
  group('vertical bar chart', () {
    test('Paint labels with default settings', () {
      ...
      final barElements = [
        ...
        FakeBarRendererElement('A', Rectangle(leftPositionA, topPositionA, barWidthA, 100 - topPositionA), (_) => 'LabelA', data),
        FakeBarRendererElement('B', Rectangle(leftPositionB, topPositionB, barWidthB, 100 - topPositionB), (_) => 'LabelB', data),
        FakeBarRendererElement('C', Rectangle(leftPositionC, topPositionC, barWidthC, 100 - topPositionC), (_) => 'LabelC', data),
      ];
      final decorator = BarLabelDecorator<String>();

      decorator.decorate(barElements, canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: true);
    ...
    test('LabelPosition.inside always paints inside the bar', () {
      ...
      ).decorate(barElements, canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: true);
    ...
    group('Null and empty label scenarios', () {
      test('Skip label if label accessor does not exist', () {
        ...
        BarLabelDecorator<String>().decorate(barElements, canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: true);
      ...
      test('Skip label if label is null or empty', () {
        final data = ['A', 'B'];
        final barElements = [FakeBarRendererElement('A', Rectangle(0, 0, 10, 20), null, data), FakeBarRendererElement('B', Rectangle(0, 50, 10, 20), (_) => '', data)];
  ...
  group('horizontal bar chart', () {
    test('Paint labels with default settings', () {
      ...
      decorator.decorate(barElements, canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: false);
    ...
    test('LabelPosition.auto paints inside bar if outside bar has less width', () {
      ...
      BarLabelDecorator<String>(
        labelPadding: 0, // Turn off label padding for testing.
        insideLabelStyleSpec: TextStyleSpec(fontSize: 10),
      ).decorate(barElements, canvas, graphicsFactory, drawBounds: smallDrawBounds, animationPercent: 1.0, renderingVertically: false);
    ...
    test('LabelPosition.inside always paints inside the bar', () {
      ...
      BarLabelDecorator<String>(
        ...
        labelPadding: 0, // Turn off label padding for testing.
        insideLabelStyleSpec: TextStyleSpec(fontSize: 10),
      ).decorate(barElements, canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: false);
    ...
    test('RTL TextAnchor.start starts on the right', () {
      ...
      BarLabelDecorator<String>(
        ...
        labelPadding: 0, // Turn off label padding for testing.
        insideLabelStyleSpec: TextStyleSpec(fontSize: 10),
      ).decorate(barElements, canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: false, rtl: true);
    ...
    group('Null and empty label scenarios', () {
      test('Skip label if label accessor does not exist', () {
        ...
        BarLabelDecorator<String>().decorate(barElements, canvas, graphicsFactory, drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: false);
