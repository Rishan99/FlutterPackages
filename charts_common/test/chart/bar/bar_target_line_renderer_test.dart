...
class MockCanvas extends Mock implements ChartCanvas {
  ...
  @override
  void drawLine({List<Point> points, Rectangle<num> clipBounds, Color fill, Color stroke, bool roundEndCaps, double strokeWidthPx, List<int> dashPattern}) {
...
void main() {
  ...
  setUp(() {
    var myFakeDesktopData = [MyRow('MyCampaign1', 5), MyRow('MyCampaign2', 25), MyRow('MyCampaign3', 100), MyRow('MyOtherCampaign', 75)];

    var myFakeTabletData = [MyRow('MyCampaign1', 5), MyRow('MyCampaign2', 25), MyRow('MyCampaign3', 100), MyRow('MyOtherCampaign', 75)];

    var myFakeMobileData = [MyRow('MyCampaign1', 5), MyRow('MyCampaign2', 25), MyRow('MyCampaign3', 100), MyRow('MyOtherCampaign', 75)];

    seriesList = [
      MutableSeries<String>(
        Series<MyRow, String>(id: 'Desktop', domainFn: (MyRow row, _) => row.campaign, measureFn: (MyRow row, _) => row.clickCount, measureOffsetFn: (MyRow row, _) => 0, data: myFakeDesktopData),
      ),
      MutableSeries<String>(
        Series<MyRow, String>(id: 'Tablet', domainFn: (MyRow row, _) => row.campaign, measureFn: (MyRow row, _) => row.clickCount, measureOffsetFn: (MyRow row, _) => 0, data: myFakeTabletData),
      ),
      MutableSeries<String>(
        Series<MyRow, String>(id: 'Mobile', domainFn: (MyRow row, _) => row.campaign, measureFn: (MyRow row, _) => row.clickCount, measureOffsetFn: (MyRow row, _) => 0, data: myFakeMobileData),
      ),
    ];
  });
  ...
  test('with stroke width target lines', () {
    renderer = makeRenderer(config: BarTargetLineRendererConfig(groupingType: BarGroupingType.grouped, strokeWidthPx: 5.0));
