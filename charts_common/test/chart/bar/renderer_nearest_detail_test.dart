...
void main() {
  ...
  MutableSeries _makeSeries({String id, String seriesCategory, bool vertical = true}) {
    ...
    final series = MutableSeries(Series<MyRow, String>(id: id, data: data, domainFn: (row, _) => row.campaign, measureFn: (row, _) => row.clickCount, seriesCategory: seriesCategory));
  ...
  MutableSeries _makeDateTimeSeries({String id, String seriesCategory, bool vertical = true}) {
    final data = <MyDateTimeRow>[MyDateTimeRow(date0, 10), MyDateTimeRow(date1, 10)];

    final series = MutableSeries(Series<MyDateTimeRow, DateTime>(id: id, data: data, domainFn: (row, _) => row.time, measureFn: (row, _) => row.clickCount, seriesCategory: seriesCategory));
  ...
  group('edge cases', () {
    test('hit target on missing data in group should highlight group', () {
      ...
      final seriesList = <MutableSeries>[_makeSeries(id: 'foo')..data.clear(), _makeSeries(id: 'bar')];
    ...
    test('all series without data is skipped', () {
      // Setup
      final renderer = _makeBarRenderer(vertical: true, groupType: BarGroupingType.grouped);
      final seriesList = <MutableSeries>[_makeSeries(id: 'foo')..data.clear(), _makeSeries(id: 'bar')..data.clear()];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
    ...
    test('single overlay series is skipped', () {
      // Setup
      final renderer = _makeBarRenderer(vertical: true, groupType: BarGroupingType.grouped);
      final seriesList = <MutableSeries>[_makeSeries(id: 'foo')..overlaySeries = true, _makeSeries(id: 'bar')];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
    ...
    test('all overlay series is skipped', () {
      // Setup
      final renderer = _makeBarRenderer(vertical: true, groupType: BarGroupingType.grouped);
      final seriesList = <MutableSeries>[_makeSeries(id: 'foo')..overlaySeries = true, _makeSeries(id: 'bar')..overlaySeries = true];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
  ...
  group('Vertical BarRenderer', () {
    ...
    test('hit test expands to grouped bars', () {
      // Setup
      final renderer = _makeBarRenderer(vertical: true, groupType: BarGroupingType.grouped);
      final seriesList = <MutableSeries>[_makeSeries(id: 'foo'), _makeSeries(id: 'bar')];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
  ...
  group('Horizontal BarRenderer', () {
    ...
    test('hit test expands to grouped bars', () {
      ...
      final seriesList = <MutableSeries>[_makeSeries(id: 'foo', vertical: false), _makeSeries(id: 'bar', vertical: false)];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
  ...
  group('with date time axis and vertical bar', () {
    ...
    test('hit test expands to grouped bars', () {
      // Setup
      final renderer = _makeBarRenderer(vertical: true, groupType: BarGroupingType.grouped);
      final seriesList = <MutableSeries>[_makeDateTimeSeries(id: 'foo'), _makeDateTimeSeries(id: 'bar')];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
