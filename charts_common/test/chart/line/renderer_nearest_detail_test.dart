...
void main() {
  ...
  MutableSeries<int> _makeSeries({String id, int measureOffset = 0}) {
    final data = <MyRow>[MyRow(1000, measureOffset + 10), MyRow(2000, measureOffset + 20), MyRow(3000, measureOffset + 30)];

    final series = MutableSeries<int>(Series<MyRow, int>(id: id, data: data, domainFn: (MyRow row, _) => row.timestamp, measureFn: (MyRow row, _) => row.clickCount));
  ...
  group('edge cases', () {
    test('hit target with missing data in series still selects others', () {
      // Setup
      final seriesList = <MutableSeries<int>>[_makeSeries(id: 'foo')..data.clear(), _makeSeries(id: 'bar')];
    ...
    test('all series without data is skipped', () {
      // Setup
      final seriesList = <MutableSeries<int>>[_makeSeries(id: 'foo')..data.clear(), _makeSeries(id: 'bar')..data.clear()];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
    ...
    test('single overlay series is skipped', () {
      // Setup
      final seriesList = <MutableSeries<int>>[_makeSeries(id: 'foo')..overlaySeries = true, _makeSeries(id: 'bar')];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
    ...
    test('all overlay series is skipped', () {
      // Setup
      final seriesList = <MutableSeries<int>>[_makeSeries(id: 'foo')..overlaySeries = true, _makeSeries(id: 'bar')..overlaySeries = true];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
  ...
  group('LineRenderer', () {
    ...
    test('hit test expands to multiple series', () {
      ...
      final seriesList = <MutableSeries<int>>[_makeSeries(id: 'foo'), _makeSeries(id: 'bar', measureOffset: 20)];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
    ...
    test('hit test expands with missing data in series', () {
      ...
      // middle point.
      final seriesList = <MutableSeries<int>>[_makeSeries(id: 'foo'), _makeSeries(id: 'bar', measureOffset: 20)..data.removeAt(1)];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
  ...
  group('Area Chart', () {
    ...
    test('hit test works for points below one series but closer to another', () {
      // Setup
      final seriesList = <MutableSeries<int>>[_makeSeries(id: 'low'), _makeSeries(id: 'middle', measureOffset: 20), _makeSeries(id: 'high', measureOffset: 40)];
      renderer.configureSeries(seriesList);
      renderer.preprocessSeries(seriesList);
      renderer.update(seriesList, false);
      renderer.paint(MockCanvas(), 1.0);
